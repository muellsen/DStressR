...
compound_label_nodes <- compound_nodes[
  compound_nodes$n_promoters >= 2 |
    compound_nodes$node_id %in% names(sort(table(hits$srn_code), decreasing = TRUE))[seq_len(min(25, length(unique(hits$srn_code))))],
]
...
p <- ggplot() +
  ...
  labs(
    title = "Consensus BH-rejected promoter-compound network",
    subtitle = "Only pairs rejected by median-polish, DStressR Gaussian, and DStressR EB"
  )

write.table(
  nodes,
  file.path(out_dir, "consensus_bh_rejected_network_nodes.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
write.table(
  ...
  file.path(out_dir, "consensus_bh_rejected_network_edges.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
ggsave(
  file.path(out_dir, "consensus_bh_rejected_promoter_compound_network.png"),
  p,
  width = 13,
  height = 11,
  dpi = 300,
...
ggsave(
  file.path(out_dir, "consensus_bh_rejected_promoter_compound_network.pdf"),
  p,
  width = 13,
  height = 11,
  bg = "white"
...
message("Wrote consensus network outputs to: ", out_dir)
message("Consensus pairs: ", nrow(hits), "; promoters: ", nrow(promoter_nodes), "; compounds: ", nrow(compound_nodes))
