...
strains to estimate perturbation-specific background Lux signal. In the original
analysis, the `PEVC3` reporter is used as the reference. For each perturbation,
the two PEVC3 replicate values are averaged, and this perturbation-specific EVC
...
`fit_empty_vector_control()` function remains available for existing scripts.
...
reporter-perturbation pair, retain the largest replicate-level p-value and apply
BH adjustment within reporter.
...
`fit_workflow(..., workflow = "empty_vector_control")` follows the original
Salmonella hit-determination logic:
...
10. Retain the largest replicate-level p-value for each reporter-perturbation pair.
...
`fit_workflow(..., workflow = "empty_vector_control")` recovers the original
`04-hit_determination/hit_table.tsv.gz` output numerically:
