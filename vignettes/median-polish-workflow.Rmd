...
For the original Campylobacter reporter-library workflow, the median-polish
...
This is a library annotation table with one row per perturbation/library well. The
...
from this table. DStressR uses the same perturbation key:
...
for each reporter-perturbation pair, and BH correction is applied within reporter.
...
1. Build `promoter_libplate_replicate = paste(reporter, libplate, replicate, sep = "_")`.
...
10. For hit calling, remove DMSO/noisy-DMSO wells, keep the largest p-value
    across replicates for each reporter-perturbation pair, and apply BH correction
    within reporter.
...
reporter-libplate-replicate group to estimate the DMSO null distribution.
...
recovers the legacy workflow, while `fit_destress()` provides the model-based
replacement.
