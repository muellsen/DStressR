...
The named workflow entry point is `fit_workflow(..., workflow = "median_polish")`.
The lower-level `fit_median_polish()` function remains available for existing
scripts.
...
If `normality = TRUE`, the function also runs the workflow's DMSO normality
screen before median polishing. The Lilliefors test uses `nortest::lillie.test()`;
