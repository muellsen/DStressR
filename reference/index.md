# Package index

## All functions

- [`add_dgrowthr_growth()`](https://bio-datascience.github.io/DStressR/reference/add_dgrowthr_growth.md)
  : Add DGrowthR-derived growth metrics to a DStressR assay table
- [`adjust_pvalues()`](https://bio-datascience.github.io/DStressR/reference/adjust_pvalues.md)
  : Adjust p-values within promoter
- [`call_hits()`](https://bio-datascience.github.io/DStressR/reference/call_hits.md)
  : Call differential stress hits
- [`destress_presets()`](https://bio-datascience.github.io/DStressR/reference/destress_presets.md)
  : List available DStressR presets
- [`destress_workflows()`](https://bio-datascience.github.io/DStressR/reference/destress_workflows.md)
  : List available DStressR workflows
- [`empirical_replicate_pvalues()`](https://bio-datascience.github.io/DStressR/reference/empirical_replicate_pvalues.md)
  : Empirical p-values from replicate-averaged compound effects and DMSO
  nulls
- [`estimate_growth_exponents()`](https://bio-datascience.github.io/DStressR/reference/estimate_growth_exponents.md)
  : Estimate promoter-specific growth normalization exponents
- [`fit_destress()`](https://bio-datascience.github.io/DStressR/reference/fit_destress.md)
  : Fit DStressR with staged statistical options
- [`fit_effect_mixture()`](https://bio-datascience.github.io/DStressR/reference/fit_effect_mixture.md)
  : Fit a three-part empirical-null mixture to promoter-compound effects
- [`fit_empty_vector_control()`](https://bio-datascience.github.io/DStressR/reference/fit_empty_vector_control.md)
  : Reproduce Empty Vector Control normalization
- [`fit_median_polish()`](https://bio-datascience.github.io/DStressR/reference/fit_median_polish.md)
  : Reproduce the legacy median-polish workflow
- [`fit_workflow()`](https://bio-datascience.github.io/DStressR/reference/fit_workflow.md)
  : Fit a named DStressR workflow
- [`model_matrix_report()`](https://bio-datascience.github.io/DStressR/reference/model_matrix_report.md)
  : Summarize model dimensions
- [`plot_effect_histogram()`](https://bio-datascience.github.io/DStressR/reference/plot_effect_histogram.md)
  : Histogram of DStressR promoter-compound effects
- [`plot_response_cluster_blocks()`](https://bio-datascience.github.io/DStressR/reference/plot_response_cluster_blocks.md)
  : Clustered block map of a DStressR promoter-by-compound response
  matrix
- [`plot_response_clustered_heatmap()`](https://bio-datascience.github.io/DStressR/reference/plot_response_clustered_heatmap.md)
  : Clustered heatmap with promoter and compound dendrograms
- [`plot_response_heatmap()`](https://bio-datascience.github.io/DStressR/reference/plot_response_heatmap.md)
  : Heatmap of a DStressR promoter-by-compound response matrix
- [`plot_volcano()`](https://bio-datascience.github.io/DStressR/reference/plot_volcano.md)
  : Volcano plot for DStressR promoter-compound hits
- [`prepare_assay()`](https://bio-datascience.github.io/DStressR/reference/prepare_assay.md)
  : Prepare a chemical-genomics assay table
- [`read_campylobacter_expression()`](https://bio-datascience.github.io/DStressR/reference/read_campylobacter_expression.md)
  : Read exported Campylobacter expression data
- [`results()`](https://bio-datascience.github.io/DStressR/reference/results.md)
  : Extract model results
- [`simulate_screen()`](https://bio-datascience.github.io/DStressR/reference/simulate_screen.md)
  : Simulate a chemical-genomics screen
